...
local fixedclasstable = {}

hook.Add("Initialize", "SetZombieClasses", function()
	for k, v in pairs(GAMEMODE.ZombieClasses) do
		if isnumber(k) or v.Boss or v.Disabled or v.Hidden then continue end
	
		table.insert(fixedclasstable, k)
	end
end)
...
function HANDLER.OnDeathFunction(bot)
	--bot:Say("rip me!")
	bot:D3bot_RerollClass(fixedclasstable) -- TODO: Situation depending reroll of the zombie class
	HANDLER.RerollTarget(bot)
end
